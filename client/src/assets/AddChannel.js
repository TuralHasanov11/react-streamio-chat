...
export const AddChannel = ({ type }) => {
  ...
  return (<svg
    ...
    onClick={() => {
      channelCtx.setCreateType(type);
      channelCtx.setIsCreating((prevState) => !prevState);
      channelCtx.setIsEditing(false);
      if(channelCtx.setToggleContainer) channelCtx.setToggleContainer((prevState) => !prevState) 
    }}
  >
    <path
